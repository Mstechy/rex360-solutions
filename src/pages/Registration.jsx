...
import { downloadRegistrationPdf } from '../utils/pdf';
...
const Registration = () => {
    ...
    try {
        ...
        setSubmissionSnapshot({
            ...
            documentCounts: Object.fromEntries(
              Object.entries(documentUrls).map(([docType, urls]) => [docType, Array.isArray(urls) ? urls.length : 0])
            ),
  ...
  const handleDownloadPdf = async () => {
    try {
      const formValues = submissionSnapshot?.fields || {};
      const documentCounts = submissionSnapshot?.documentCounts || {};

      await downloadRegistrationPdf({
        filename: `${(formValues.surname || getValue('surname') || 'registration')}_${(formValues.firstname || getValue('firstname') || 'file')}_registration.pdf`,
        title: 'REX360 Solutions Registration Summary',
        subtitle: `Service: ${submissionSnapshot?.serviceType || serviceType}`,
        sections: [
          {
            title: 'Applicant Details',
            rows: [
              { label: 'Service Type', value: submissionSnapshot?.serviceType || serviceType },
              { label: 'Surname', value: formValues.surname || getValue('surname') },
              { label: 'First Name', value: formValues.firstname || getValue('firstname') },
              { label: 'Email', value: formValues.email || getValue('email') },
              { label: 'Phone', value: formValues.phone || getValue('phone') },
              { label: 'Payment Reference', value: submissionSnapshot?.paymentReference || paymentReference || 'N/A' },
              { label: 'Payment Status', value: 'Paid' },
              { label: 'Amount Paid', value: `₦${Number(submissionSnapshot?.amount || currentPrice || 0).toLocaleString()}` },
            ],
          },
          {
            title: 'Business Details',
            rows: [
              { label: 'Business Category', value: submissionSnapshot?.category || category || 'N/A' },
              { label: 'Business Nature', value: submissionSnapshot?.nature || nature || 'N/A' },
            ],
          },
          {
            title: 'Uploaded Documents',
            rows: Object.keys(files).map((docType) => ({
              label: docType,
              value: `${documentCounts[docType] || files[docType]?.length || 0} file(s)`,
            })),
          },
        ],
      });
        ...
        <div className="max-w-2xl relative z-10">
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              className="inline-flex items-center justify-center bg-slate-900 text-white px-10 py-4 rounded-full font-black text-lg shadow-xl hover:scale-105 transition-all transform"
            >
              <FileText className="mr-2" size={24} /> DOWNLOAD PDF COPY
            </button>
