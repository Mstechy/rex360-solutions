  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      await downloadRegistrationPdf({
        filename: `${(snapshot.fields?.surname || formData.surname || 'registration')}_${(snapshot.fields?.firstname || formData.firstname || 'file')}_registration.pdf`,
        title: 'REX360 Solutions Registration Summary',
        subtitle: `Service: ${snapshot.serviceType || serviceType}`,
        sections: [
          {
            title: 'Applicant Details',
            rows: [
              { label: 'Service Type', value: snapshot.serviceType || serviceType },
              { label: 'Surname', value: snapshot.fields?.surname || formData.surname },
              { label: 'First Name', value: snapshot.fields?.firstname || formData.firstname },
              { label: 'Other Name', value: snapshot.fields?.othername || formData.othername },
              { label: 'Email', value: snapshot.fields?.email || formData.email },
              { label: 'Phone', value: snapshot.fields?.phone || formData.phone },
              { label: 'Payment Reference', value: snapshot.paymentReference || paymentReference || 'N/A' },
              { label: 'Payment Status', value: 'Paid' },
              { label: 'Amount Paid', value: `₦${Number(snapshot.amount || currentPrice || 0).toLocaleString()}` },
            ],
          },
          {
            title: 'Business Details',
            rows: [
              { label: 'Business Category', value: snapshot.category || category || 'N/A' },
              { label: 'Business Nature', value: snapshot.nature || nature || 'N/A' },
            ],
          },
          {
            ...
            rows: Object.keys(files).map((docType) => ({
              label: docType,
              value: `${snapshot.documentCounts?.[docType] || files[docType]?.length || 0} file(s)`,
            })),
          },
        ],
      });
        ...
        <div className="max-w-2xl">
          ...
          <div className="mt-8">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full bg-blue-700 text-white font-semibold shadow-lg hover:bg-blue-800 transition-all"
            >
              <FileText size={18} />
              Download PDF Copy
            </button>
